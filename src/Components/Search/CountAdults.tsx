...
import {addAdultsActionCreator, addKidsActionCreator} from "../../redux/searchReducer";
import {getAdults} from "../../Selectors/selectors";
...
export let CountAdults: FC<PropsType> = (props) => {
    const dispatch = useDispatch()
    const adultsProps = useSelector(getAdults)
    ...
    const [adults, setAdults] = useState(adultsProps)
    useEffect(() => {
        addAdults(adults)
    }, [adults])
    ...
    if (adults === 0) {
        return <div className='settings_block'><span className='settings_block-text'>Дорослих</span>
            ...
            <span className='settings_block-number'>{adults}</span>
